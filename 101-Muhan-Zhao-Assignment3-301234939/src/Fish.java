...
{
	protected double scalex, scaley, rotation;
	...
	public Fish(Fish target)
	{
		...
		oscillation = target.oscillation;
	...
	private boolean canEats(Fish target)
	{
		if(handleCollision(target) && getDirection() != target.getDirection())
		{
			return true;
		}
...
//		}
	...
	{
		if (posx+myWidth/2+offset >= FishPanel.WIN_W || posx-myWidth/2-offset <= 0)
		{
			if (getDirection() > 0)
			{
				posx = FishPanel.WIN_W - offset-myWidth/2;				
			}
			else
			{
				posx = offset+myWidth/2;
			}
			return true;
		}
		return false;
...
//		g2.fillOval((int) (myWidth/2+offset), 0, 10, 10);
//		g2.fillOval((int) (-myWidth/2-offset), 0, 10, 10);
